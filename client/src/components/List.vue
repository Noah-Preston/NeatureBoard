<template>
  <div class="col-11 col-md-3 card list-card mx-3" @dragover.prevent @drop.prevent="moveTask">
    ...
    <div class="row">
      <div class="col-3">
        <form @submit.prevent="addTask" class="form-width">
          <input type="text" placeholder=" add a new task" v-model="newTask.content" required class="rounded form-width"
            id="formv" />
          <button type="submit" class="btn btn-sm btn-info">+</button>
        </form>
        <div class="row text-center align-items-center">
          <Task v-for="(taskObj, index) in tasks" :key="taskObj.id" :taskData="taskObj" :taskIndex="index" />
        </div>
      </div>
    </div>
    <div class="row h-100">
